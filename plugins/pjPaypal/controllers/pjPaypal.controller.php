...
class pjPaypal extends pjPaypalAppController
{
	...
	public function pjActionConfirm()
	{
		...
		if (!empty($errstr))
		{
            $this->log($this->logPrefix . "Error: {$errstr}");
		}
		...
		if (!$fp)
		{
            $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>HTTP error: {$errstr}");
		} else {
			...
			while (!feof($fp))
			{
				...
				{
                    $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>VERIFIED");
					if ($payment_status == "Completed")
					{
                        $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>Completed");
						if (!in_array($txn_id, $params['txn_id']))
						{
                            $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>TXN_ID is OK");
							if ($receiver_email == $options['merchant_email'])
							{
                                $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>EMAIL address is OK");
								if ($payment_amount == $params['amount'])
								{
                                    $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>AMOUNT is OK");
									if ($payment_currency == $this->option_arr['o_currency'])
									{
                                        ...
                                        $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>CURRENCY didn't match");
									}
								} else {
                                    $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>AMOUNT didn't match: {$payment_amount} != {$params['amount']}");
								}
							} else {
                                $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>EMAIL address didn't match");
							}
						} else {
                            $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>TXN_ID is the same.");
						}
					} else {
						$this->log($this->logPrefix . "TXN ID: {$txn_id}<br>Not Completed");
					}
			    } elseif (strcasecmp($buffer, "INVALID") == 0) {
                    $this->log($this->logPrefix . "TXN ID: {$txn_id}<br>INVALID");
			  	}
			}
			fclose($fp);
		}
