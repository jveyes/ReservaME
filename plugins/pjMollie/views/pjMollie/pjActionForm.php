<form action="" method="post" name="plugin_mollie_payment_form" id="plugin_mollie_payment_form" target="<?php echo $tpl['arr']['target']; ?>"></form>
...
<div class="modal fade" id="modalMollie" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-md" role="document">
		<form id="<?php echo pjSanitize::html(@$tpl['arr']['id']); ?>" action="<?php echo pjSanitize::html(@$tpl['arr']['notify_url']) . '&id=' . pjSanitize::html(@$tpl['arr']['custom']); ?>" method="post" class="form-horizontal modal-content">
			...
			<div class="modal-body">
				...
				<?php if(array_key_exists('ideal', $methods)): ?>
					<?php $issuers = $controller->requestAction(array('controller' => 'pjMollie', 'action' => 'pjActionGetAvailUsers', 'params' => array('public_key' => $tpl['arr']['public_key'])), array('return')); ?>
	...
	if (form) {
		form.validate({
			...
			submitHandler: function (form) {
				$.post(['<?php echo PJ_INSTALL_URL; ?>', "index.php?controller=pjMollie&action=pjActionGetUrl"].join(""), $(form).serialize()).done(function (data) {
		...
		if (form.find('select[name="mollie_method"] option[data-auto-submit="1"]').length > 0)
		{
