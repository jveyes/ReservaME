 ...
 */
	public function init($renew=null)
	{
		...
		if (empty($_SESSION[$this->sessionVariable]))
		{
			switch ($this->mode)
			{
				case 'addition':
					...
					$_SESSION[$this->sessionVariable] = $correct;
        ...
        $xRepeat = $this->mode == 'string' && $this->length > 7? ceil($this->length / 7): 1;
        $this->setWidth($this->width * $xRepeat);

		if (!is_null($this->image))
		{
			if ($xRepeat > 1)
            {
                // Create a new image by copying the original pattern horizontally multiple times ($xRepeat).
                $src = imagecreatefrompng($this->image);
                $image = imagecreatetruecolor($this->width, $this->height);
                $current_x = 0;
                while($current_x < $this->width){
                    imagecopy($image, $src, $current_x, 0, 0, 0, $originalWidth, $this->height);
                    $current_x += $originalWidth;
                }
                imagedestroy($src);
            }
            else
		...
		// Use lower angle for longer strings to prevent text from hiding.
		if ($this->length <= 10)
		{
			$angle = rand(-10, 10);
		} elseif ($this->length > 10 && $this->length <= 13) {
			$angle = rand(-7, 7);
		} elseif ($this->length > 13 && $this->length <= 16) {
			$angle = rand(-5, 5);
		} else {
			$angle = rand(-4, 4);
		}
