...
class pjPHPMailer
{
     ...
     *
     * @see pjPHPMailer::$Username
     * @see pjPHPMailer::$Password
     *
     * @var bool
     */
     ...
     *
     * @see pjPHPMailer::validateAddress()
     *
     * @var string|callable
     */
     ...
     * @see pjPHPMailer::$cc
     * @see pjPHPMailer::$bcc
     * @see pjPHPMailer::$all_recipients
     *
     * @var array
     */
     ...
     *
     * @see pjPHPMailer::$Debugoutput
     * @see pjPHPMailer::$SMTPDebug
     *
     * @param string $str
     */
    ...
    protected function addOrEnqueueAnAddress($kind, $address, $name)
    {
        ...
        if (false === $pos) {
            ...
            if ($this->exceptions) {
                throw new pjException($error_message);
            }
            ...
            if (ini_get('mail.add_x_header') == 1
                ...
                try {
                    ...
                    if (count($this->to) + count($this->cc) + count($this->bcc) < 1) {
                        throw new pjException($this->lang('provide_address'), self::STOP_CRITICAL);
                    }
                    ...
                    if (!$this->AllowEmpty and empty($this->Body)) {
                        throw new pjException($this->lang('empty_message'), self::STOP_CRITICAL);
                    }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new pjException($this->lang('execute') . $this->Sendmail, self::STOP_CRITICAL);
                }
    ...
    protected function mailSend($header, $body)
    {
        ...
        if (!$result) {
            throw new pjException($this->lang('instantiate'), self::STOP_CRITICAL);
        }
     ...
     * @return pjSMTP
     */
    public function setSMTPInstance(pjSmtp $smtp)
    {
        $this->smtp = $smtp;
     ...
     *
     * @see pjPHPMailer::setSMTPInstance() to use a different class.
     *
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect()) {
            throw new pjException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
        }
        ...
        if (!$this->smtp->mail($smtp_from)) {
            ...
            throw new pjException($this->ErrorInfo, self::STOP_CRITICAL);
        }
        ...
        if ((count($this->all_recipients) > count($bad_rcpt)) and !$this->smtp->data($header . $body)) {
            throw new pjException($this->lang('data_not_accepted'), self::STOP_CRITICAL);
        }
    ...
    public function smtpConnect($options = null)
    {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (!preg_match(
                ...
                // On PHP ver. higher than 5.4.8 check for an OpenSSL constant rather than using extension_loaded, which is sometimes disabled
                $sslext = version_compare(PHP_VERSION, '5.4.8', '<')? extension_loaded('openssl'): defined('OPENSSL_ALGO_SHA256');
                if ('tls' === $secure or 'ssl' === $secure) {
                    if (!$sslext) {
                        throw new pjException($this->lang('extension_missing') . 'openssl', self::STOP_CRITICAL);
                    }
                }
                $host = $hostinfo[3];
                $port = $this->Port;
                    ...
                    try {
                        ...
                        if ($tls) {
                            if (!$this->smtp->startTLS()) {
                                throw new pjException($this->lang('connect_host'));
                            }
                        ...
                        if ($this->SMTPAuth) {
                            if (!$this->smtp->authenticate(
                                ...
                                $this->oauth
                                    ...
                                    throw new pjException($this->lang('authenticate'));
                                }
                        }
        ...
        if ($this->isError()) {
            ...
            try {
                if (!defined('PKCS7_TEXT')) {
                    throw new pjException($this->lang('extension_missing') . 'openssl');
                }
                // @TODO would be nice to use php://temp streams here
                $file = tempnam(sys_get_temp_dir(), 'mail');
                if (false === file_put_contents($file, $body)) {
                    throw new pjException($this->lang('signing') . ' Could not write temp file');
                }
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new pjException($this->lang('signing') . openssl_error_string());
                }
            } catch (Exception $exc) {
                $body = '';
                if ($this->exceptions) {
    ...
    public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment')
    {
        try {
            if (!@is_file($path)) {
                throw new pjException($this->lang('file_access') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            if (!file_exists($path)) {
                throw new pjException($this->lang('file_open') . $path, self::STOP_CONTINUE);
            }
    ...
    protected function doCallback($isSent, $to, $cc, $bcc, $subject, $body, $from, $extra)
    {
        if (!empty($this->action_function) and is_callable($this->action_function)) {
            call_user_func_array($this->action_function, array($isSent, $to, $cc, $bcc, $subject, $body, $from, $extra));
        }
    }
}
?>